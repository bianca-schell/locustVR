...
projectDB = 'locustProjects_2_3post-june.db'   #'locustProjects_2post_2m_deg30_45_60.db'
expDB = 'locustExperiments_2_3post-june.db' 	#'locustExperiments_2post_2m_deg30_45_60.db'
...
posts = list(itertools.chain.from_iterable(itertools.repeat(x, 10) for x in posts)) #original: 10, is this to change size of db?
distances = [3.0]	#in meter
start_ang_split = 16
angles2 = [np.pi*35/180, np.pi*35/180,np.pi*35/180] #[np.pi/6, np.pi/4, np.pi]			#degree: 60, 70, 180:  [np.pi/3, 7*np.pi/18, np.pi]
angles3 = [np.pi*35/180 , np.pi*35/180,np.pi*35/180]#[4.5*np.pi/18, 3*np.pi/18, 2*np.pi/3] #original: angles3 = [5*np.pi/18, 5*np.pi/18, 2*np.pi/3] #changed: 45,30,120
