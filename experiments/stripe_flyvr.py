...
class MyExperiment(object):
    ...
    def updateStimuli(self,nStimuli):
        ...
        # connect a cursor that goes through the project database
        cursorProject = conn.cursor()
        # pick a new background cube from available 
        for nCube in range (0, 3):
            ...
            if data == 'None':
                ...
                self.postPosition[nCube,:] = [1000,1000]
            else:
                #dictData = eval(data)
                #print(dictData['position'])
                self.postPosition[nCube,:] = [0,0]
            self.ds_proxy.move_node('Cube' + str(nCube), self.postPosition[nPost,0],  self.postPosition[nPost,1], 0)
