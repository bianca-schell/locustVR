    ...
    def running_writing_csv(self):
        ...
        with open(path+'/results.csv', 'a') as output:
            ...
            while nStimuli<=4:
                ...
                if nStimuli==0 and reset==False:
                    self.reset_origin()
                    self.updateStimuli(nStimuli)
                    reset=True
                    #after 3 min *****WITHOUT STIMULUS**, locust is reset to origin (0,0) 


                if nStimuli==0 and t>5 and firstinitialized==False:
                    #change to t>3*60!!!
                    nStimuli += 1
                ...
                if self.locPosition['x'] == 0.0 and write==True:
                    #info that its really reset to 0,0 and gives post position
                ...
                if nStimuli>=1:
                    ...
                    for nPost in range(0,10):
                        ...
                        if dist < 4.85 and reached == False:
                            #change to dist < 0.5!!!
                            write=True
                            ...
                            self.updateStimuli(nStimuli)
                            self.reset_origin()
                        if self.locPosition['x']==0.0 and self.locPosition['y']==0:
                            reached=False
