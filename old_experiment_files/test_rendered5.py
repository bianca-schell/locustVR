        ...
        try:
            while 1:
                ...
                if (t - t0) > 1:
                    ...
                    #hier aendern!!!
                    #e.do_move_world(10, 10,0) funktioniert nicht
                    
                    #self.reset_origin()
                    if (t - t0) > 5:
                        t0 = t
                        e.load_osg('/home/loopbio/Documents/stimuli/ten_post_stimulus.osgt')
                        for l in range(0,4):
                            l.move(x=5,Y=5,z=0)

#wie setze ich l als variable der class: class _OSG2FileHandle(object): aus stimulus_osg????*********
                        ...
                        #getExp necessary?no
                        #for nPost in range(0,10):
                         #   self.ds_proxy.move_node('Cylinder' + str(nPost), self.postPosition[nPost,5],  self.postPosition[nPost,10], 0)
        except KeyboardInterrupt:
            self.stop()
      
class Working_class(StimulusOSGController):
    #def __init__(self):
    pass
